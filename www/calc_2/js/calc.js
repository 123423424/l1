...
jQuery(function()
{
	...
	function price_color() {
		...
		switch(textC) {
			...
			case 'fff324': //yellow было cprice = 1570;
				cprice = 1300;
			break;
			case 'colored':  // было cprice = 2900; это цветной цвет RGB  
				cprice = 2500;
			break;
    ...
    function makePrice() {
       ...
       */ 
        ...
        /****** НОВЫЙ КОД ******/
        //Зависимость RGB цвета от количества       
        if(textC == 'colored') {if(space < 4608) {price = setPrise(0, 3)}  else { if(space < 9216) {price = setPrise((space/512*(29*getCbris())), 1) }   else {   price = setPrise((space/512*(27*getCbris())), 1) }}};      
         /* КОНЕЦ */ 
